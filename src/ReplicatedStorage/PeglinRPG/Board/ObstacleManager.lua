...
function ObstacleManager:addSpecialObstacles(board, width, height, theme)
    -- Definir configuraciones por defecto en caso de que no existan en Config
    ...
    -- Verificar si Config.BOARD_ELEMENTS existe
    local boardElements = {}
    if Config.BOARD_ELEMENTS then
        boardElements = Config.BOARD_ELEMENTS
    else
        boardElements = defaultSettings
    end
    
    -- Crear deflectores (bumpers)
    local bumperSettings = boardElements.BUMPERS or defaultSettings.BUMPERS
    self:createBumpers(board, width, height, theme, bumperSettings)
    
    -- Crear paredes internas
    local wallSettings = boardElements.INTERNAL_WALLS or defaultSettings.INTERNAL_WALLS
    self:createInternalWalls(board, width, height, theme, wallSettings)
    
    -- Crear zonas de multiplicador
    local zoneSettings = boardElements.MULTIPLIER_ZONES or defaultSettings.MULTIPLIER_ZONES
    self:createMultiplierZones(board, width, height, theme, zoneSettings)
    
    -- Crear canales de aceleración
    local laneSettings = boardElements.SPEED_LANES or defaultSettings.SPEED_LANES
    self:createSpeedLanes(board, width, height, theme, laneSettings)
    
    -- Crear portales de teletransporte (para temas avanzados como "DUNGEON")
    if theme == "DUNGEON" then
        local portalSettings = boardElements.TELEPORT_PORTALS or defaultSettings.TELEPORT_PORTALS
        self:createTeleportPortals(board, width, height, portalSettings)
    end
end

-- Función para crear bumpers (deflectores) que dan más puntos al golpearlos
function ObstacleManager:createBumpers(board, width, height, theme, bumperSettings)
    ...
    -- Crear bumpers en posiciones estratégicas
    ...
    for i = 1, bumperCount do
        ...
        if validPosition then
            ...
            bumper:SetAttribute("DamageMultiplier", 1.5) -- Valor por defecto
            bumper:SetAttribute("HitCount", 0)
            bumper:SetAttribute("MaxHits", 1) -- Los bumpers suelen tener 1 golpe
...
function ObstacleManager:createInternalWalls(board, width, height, theme, wallSettings)
    -- Determinar aspecto según tema
    local wallColor
    local wallMaterial
    ...
    -- Crear paredes diagonales/horizontales internas
...
function ObstacleManager:createMultiplierZones(board, width, height, theme, zoneSettings)
    -- Crear zonas de multiplicador
...
function ObstacleManager:createSpeedLanes(board, width, height, theme, laneSettings)
    -- Crear carriles de aceleración
    ...
    for i = 1, laneCount do
        ...
        for j = 0, segmentCount - 1 do
            ...
            -- En lugar de guardar un diccionario, guarda los componentes individualmente
            arrow:SetAttribute("DirectionX", direction.X)
            arrow:SetAttribute("DirectionY", direction.Y)
            arrow:SetAttribute("DirectionZ", direction.Z)
...
-- Función para crear portales de teletransporte
function ObstacleManager:createTeleportPortals(board, width, height, portalSettings)
    -- Crear pares de portales
    ...
    for i = 1, portalPairs do
        ...
        -- En lugar de guardar un diccionario, guarda los componentes individualmente
        entryPortal:SetAttribute("ExitPositionX", exitX)
        entryPortal:SetAttribute("ExitPositionY", exitY)
        entryPortal:SetAttribute("ExitPositionZ", 0)
